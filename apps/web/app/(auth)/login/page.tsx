...
export default function LoginPage() {
  ...
  const [pendingCaptcha, setPendingCaptcha] = useState<{ id: string; answer: number } | null>(null);
  ...
  const { login, isLoginPending } = useAuth({
    onCaptchaRequired: () => setShowCaptcha(true),
  });
  ...
  const onSubmit = (data: LoginFormValues) => {
    ...
    if (pendingCaptcha) {
      payload.captcha_id = pendingCaptcha.id;
      payload.captcha_answer = pendingCaptcha.answer;
    }

    login(payload);
  };

  const handleCaptchaVerified = (captchaId: string, answer: number) => {
    setPendingCaptcha({ id: captchaId, answer });
