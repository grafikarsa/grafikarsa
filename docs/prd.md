...
Tech Stack: 
- Frontend: Next.js (React) + Tailwind CSS + Shadcn UI + TypeScript + Base UI + Hugeicons
...
### 1. Slug & Username Policy
- **Username Uniqueness**: Username harus unik secara global.
- **Reserved Usernames**: `admin`, `dashboard`, `login`, `register`, `api`, `feed`, `explore`, `search`, `settings`, `profile`, `logout`, `grafikarsa`. User dilarang menggunakan username ini.
- **Portfolio Slug**: Format slug portofolio: `base_url/[username]/[slug-judul-karya]`.
  - Slug judul karya harus unik **per user**, bukan global.
  - Contoh: `grafikarsa.com/budi/mesin-waktu` dan `grafikarsa.com/anton/mesin-waktu` adalah URL yang valid dan berbeda.

### 2. Image Optimization & Limits (MinIO)
- **Max File Size**: 
  - Avatar & Banner: Max 2MB
  - Portfolio Images: Max 5MB per file
- **Supported Formats**: JPG, PNG, WebP
- **Optimization**: Client-side compression sebelum upload (jika memungkinkan) atau server-side processing untuk resize/convert ke WebP.

### 3. Data Seeding & Initial Setup
- **Super Admin Creation**: Karena register ditiadakan, sistem harus memiliki mekanisme `seed` atau CLI command untuk membuat akun Super Admin pertama.
  - Command contoh: `go run main.go seed:admin --email=admin@grafikarsa.com --password=secret`
- **Initial Data**: Script seeding juga harus mengisi data master awal seperti Jurusan default (RPL (Rekayasa Perangkat Lunak), TKJ (Teknik Komputer dan Informatika), DKV (Desain Komunikasi Visual), ANI (Animasi), PH (Perhotelan), TG (Teknik Grafika), TL (Teknik Logistik), MEKA (Mekatronika)) dan Tahun Ajaran aktif (2024/2025).

### 4. Security & Limitations
- **Password Reset**: Tidak ada fitur "Forgot Password" otomatis di sistem. User yang lupa password harus menghubungi Admin secara manual (via WhatsApp/Email) untuk reset password.
- **Portfolio Upload Limit**: 
  - Total Portofolio: Unlimited (tidak dibatasi).
  - Rate Limit: Maksimal membuat 10 portofolio baru per hari per user (untuk mencegah spam).
- **Video Content**: Video **Wajib Embed** (YouTube). User tidak bisa mengupload file video langsung ke server (untuk menghemat bandwidth & storage).
- **Notifikasi**: Tidak ada fitur notifikasi sistem (MVP). User cek status portofolio secara manual di dashboard/profil mereka.
...
- Panel Admin:  
  ...
  - Kelola Tahun Ajaran (CRUD):  
    ...
    - promotion\_day (tanggal naik kelas otomatis. auto promotion/kenaikan kelas siswa akan ditangani oleh cronjob)  
    - **Mekanisme Cronjob Kenaikan Kelas & Kelulusan:**
      - Saat tanggal promotion tiba, sistem akan memproses semua siswa aktif.
      - **Kenaikan Kelas (10 ke 11, 11 ke 12):** Siswa dipromosikan ke tingkat berikutnya, dibuatkan entry baru di `student_class_history` (catatan: perlu tabel `student_class_history` untuk mencatat riwayat kelas).
      - **Kelulusan (12 ke Alumni):** Siswa kelas 12 akan otomatis diubah role-nya menjadi `alumni`, status `graduated`, dan tahun lulus diisi.
      - **Drop Out / Keluar:** Admin dapat mengubah status siswa menjadi `dropped_out` atau `inactive` secara manual jika siswa keluar sebelum lulus.
  - Kelola Kelas:
  ...
  - Kelola User:  
    ...
    - status (active, graduated, dropped_out, inactive) - default: active
  ...
  - Kelola Portfolio User (CRUD):  
    ...
    - thumbnail  
    - **Sistem Versioning Portofolio (Hanya 2 Versi):**
      - Sistem menyimpan maksimal 2 versi portofolio per user: **Versi Live (Published)** dan **Versi Draft/Pending (Sedang diedit)**.
      - Jika user mengedit portofolio yang sudah published:
        - Versi published **tetap live** dan tampil di publik.
        - Sistem membuat **versi draft baru** untuk diedit user.
        - Setelah draft disubmit dan disetujui admin, versi draft menggantikan versi published (versi lama di-archive/dihapus).
        - Jika ditolak, status draft menjadi `rejected` dan user bisa revisi ulang.
    - status (draft, pending_review, rejected, published, archived)  
      - status draft → draft versi baru, belum dikirim.
      - status pending_review → menunggu moderasi admin.
      - status rejected → ditolak admin, perlu revisi.
      - status published → versi aktif yang tampil di publik.
      - status archived → versi lama yang tidak aktif.  
    ...
    - dimana user dapat mengklik tombol edit profil yang akan mengarahkan user ke halaman edit profil dia:  
      - yang bisa diedit oleh user:  
        ...
        - social profile link:  
          ...
          - medium  
          - gitlab  
    - dihalaman profil user (base_url/[username]) tampilannya akan ada:  
      - banner  
      - foto profil  
      - nama  
      - username  
      ...
      - list card portofolio user tersebut, dengan aturan:  
        - halaman "base_url/[username]" (profil milik user tersebut yang sedang login) menampilkan semua portofolio milik user tersebut (draft, pending_review, rejected, published, archived)  
